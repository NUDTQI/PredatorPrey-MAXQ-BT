...
public:
	...
	CAction(ActionType type,int act,int index){m_type = type;m_eSelectedAction = act;pActionData=NULL;m_iActionIndex=index;}
	~CAction(void){};

protected:
	//action type
	ActionType m_type;
	//action index in current action set
	int m_iActionIndex;
	//action index in defination
...
public:
	...
	int getActionIndex(){return m_iActionIndex;}
